...
namespace CCC.Runtime.SceneLoading
{
	...
	public class SceneLoader : MonoBehaviour
	{
		...
		private float _sceneSwitchFadeDuration = 2;
		...
		public UniTask ResetScenes()
		{
			...
			List<SceneEntry> scenesToLoad = Scenes.Where(scene => scene.loadOnStart && !IsSceneLoaded(scene)).ToList();
		...
		{
			await UniTaskUtils.Interpolate(EntryTransition, _sceneSwitchFadeDuration, ignoreTimeScale: true);
			UniTask minLoadTimer = UniTaskUtils.Delay(_minLoadTime);
			...
			await UniTaskUtils.Interpolate(ExitTransition, _sceneSwitchFadeDuration, ignoreTimeScale: true);
		}
